...
//        User user=userDao.findByName(loginName);// 实际项目中，这里可以根据实际情况做缓存，如果不做，Shiro自己也是有时间间隔机制，2分钟内不会重复执行该方法
        AuthInfoDTO authDTO = authService.findAuthInfoByLoginName(loginName);
        if (authDTO != null) {
            //权限信息对象info,用来存放查出的用户的所有的角色（role）及权限（permission）
            SimpleAuthorizationInfo info = new SimpleAuthorizationInfo();
            //用户的角色集合
            info.setRoles(authDTO.getRoles());
            //用户的角色对应的所有权限，如果只使用角色定义访问权限，下面的四行可以不要
            info.setStringPermissions(authDTO.getPermissions());
...
//        logger.info("验证当前Subject时获取到token为：" + ReflectionToStringBuilder.toString(token, ToStringStyle.MULTI_LINE_STYLE));
        ...
        //查出是否有此用户
        AuthDTO user = authService.findAuthDTO(token.getUsername());
        if (user != null) {
            // 若存在，将此用户存放到登录认证info中，无需自己做密码对比，Shiro会为我们进行密码对比校验

//            return new SimpleAuthenticationInfo(user.getLoginName(), user.getPwd(), getName());
            return new SimpleAuthenticationInfo(user.getLoginName(), user.getPwd() + ":" + user.getPwdslat(), getName());
        }
        return null;
    }
}
